...
def create_materials( massDensity=0.0072, youngsModulus=82e9, poissonRatio=0.28 ):
    mdb.models['Model-1'].Material(description='Units - g-mm-s', name='Brass')
    mdb.models['Model-1'].materials['Brass'].Density(table=((massDensity, ), ))
    mdb.models['Model-1'].materials['Brass'].Elastic(table=((youngsModulus, poissonRatio), ))
    ## Create / Assign Section
    mdb.models['Model-1'].HomogeneousSolidSection(material='Brass', name='Section-1', thickness=None)
    mdb.models['Model-1'].parts['Tuning_Fork'].SectionAssignment(offset=0.0, offsetField='', offsetType=MIDDLE_SURFACE, region=mdb.models['Model-1'].parts['Tuning_Fork'].sets['WholePart'], sectionName='Section-1', thicknessAssignment=FROM_SECTION)

def create_assembly( ):
    mdb.models['Model-1'].rootAssembly.DatumCsysByDefault(CARTESIAN)
    mdb.models['Model-1'].rootAssembly.Instance(dependent=ON, name='Tuning_Fork-1', part=mdb.models['Model-1'].parts['Tuning_Fork'])

def create_step( ):
    mdb.models['Model-1'].FrequencyStep(limitSavedEigenvectorRegion=None, name='Step-1', numEigen=10, previous='Initial')

def create_boundary_conditions( ):
    mdb.models['Model-1'].EncastreBC(createStepName='Initial', localCsys=None, name='HoldStem', region=mdb.models['Model-1'].rootAssembly.instances['Tuning_Fork-1'].sets['HoldStem'])
    mdb.models['Model-1'].DisplacementBC(amplitude=UNSET, createStepName='Step-1', distributionType=UNIFORM, fieldName='', fixed=OFF, localCsys=None, name='Hold_YMax', region=mdb.models['Model-1'].rootAssembly.instances['Tuning_Fork-1'].sets['YMax'], u1=UNSET, u2=0.0, u3=UNSET, ur1=UNSET, ur2=UNSET, ur3=UNSET)
    mdb.models['Model-1'].DisplacementBC(amplitude=UNSET, createStepName='Step-1', distributionType=UNIFORM, fieldName='', fixed=OFF, localCsys=None, name='Hold_YMin', region=mdb.models['Model-1'].rootAssembly.instances['Tuning_Fork-1'].sets['YMin'], u1=UNSET, u2=0.0, u3=UNSET, ur1=UNSET, ur2=UNSET, ur3=UNSET)
...
def extract_frequency(odbPath, odbName="TuningFork_Eigen"):
    ...
    frequency = odb.steps['Step-1'].frames[2].frequency
    odb.close()
    return frequency
